...
**Remaining P0 emit points (registered, not yet wired):** the compliance counters (crisis/refusal/consent/deletion), `signup_attribution`, `reveal_viewed`, `reaudit_delta`, `stage_completed`, and the mapper events — the funnel backbone, outcome idempotency, close-the-loop, and engagement/accuracy events are live.
