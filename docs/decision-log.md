...
- **D4** — ship option (a): structured taps only; free text near cards gets a script-voiced "tap to confirm" nudge. Option (b) (pre-select+confirm from free text) is a scaffolded fast-follow — the mapping seam + `TODO(phase-b)` are left in `ThreadVerification`.
