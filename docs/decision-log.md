...
**Amendment (2026-06-27, DL-79):** the **Anthropic** BAA drops off this list — Claude is now called via **Azure AI Foundry** (DL-79) and the signed **Azure** BAA covers Claude-in-Foundry, so Anthropic no longer receives PHI directly. Azure remains on the list (now covering Claude too). Remaining chain: Azure (incl. Claude), AWS, Voyage AI, 1upHealth.
...
**Amendment (2026-06-26, Brock):** the materiality rule gains a **$25 absolute floor**. Final rule: a gap is material when **gap > $25 (absolute) OR (gap > $1 AND > 5%)**. The $25 floor guarantees a materially large dollar gap can never be masked by the percentage test (e.g. $30 on a $5,000 deductible = 0.6%). Brock sets the audit sensitivity; $25 is his call.
...
## DL-79 — Claude is called via Azure AI Foundry (managed identity); Azure BAA covers Claude instead of an Anthropic BAA

**Date:** 2026-06-27
**Decided by:** Phil (CTO)
**Decision:** The runtime calls Claude through **Azure AI Foundry** — the Anthropic **Messages API** exposed at `https://{resource}.services.ai.azure.com/anthropic` — rather than the Anthropic API directly. The runtime authenticates with its **managed identity (Entra)**, not an API key: the runtime Container App's user-assigned identity (`azurerm_user_assigned_identity.runtime`) is granted **Cognitive Services User** on the Foundry AI-Services resource, and `_client()` attaches an Entra bearer token (scope `https://cognitiveservices.azure.com/.default`) to the Anthropic SDK pointed at the Foundry base URL. Because the Anthropic Messages API is compatible, the change is a client-construction refactor, not a rewrite. **Foundry is the primary, BAA-covered path**; the direct Anthropic API is demoted to a **config-gated dev / emergency fallback** (`use_foundry` default true in prod; a prod assertion forbids routing PHI to Anthropic-direct). Claude is deployed **US Data Zone Standard** (US data residency) in a Claude-supported region. **Rationale:** this lets **Azure's DPA BAA** cover Claude usage — Anthropic no longer receives PHI directly — so the project does not need a separate Anthropic BAA (amends DL-49).
**BAA status:** the signed Azure BAA is assumed to cover Claude-in-Foundry (Brock's call). This is a **dev** environment with **no real PHI**, so PHI traffic is **not** separately gated. When production stands up with real PHI, re-confirm the Azure BAA covers Claude-in-Foundry before go-live.
**Reasoning:** managed identity removes a long-lived key (better HIPAA posture); Foundry + Azure DPA collapses the Claude-BAA question into the Azure BAA we already have, removing the Anthropic-BAA long pole; the Messages-API compatibility keeps the change small and reversible.
**Reversibility:** the managed-identity + Foundry-primary + Anthropic-direct-demoted structure is locked; the model/region/deployment-type are operational and tunable; if Azure ever declines BAA for Claude-in-Foundry, revert to the Anthropic-direct + Anthropic-BAA path (DL-49 as originally written).

## DL-80 — Accumulator engine wired into the audit path (orchestrator pre-agent step; context injection to Math Person)
...
## DL-90 — plan_types_bound / coverage_regime v2: the 14-value enum + coverage attributes

**Date:** 2026-07-06 · **Decided by:** Brock (memo `insurance_type_enum_research_2026-07-06.md`) · **Sprint:** Batch-1 (gates Josh's 50-state seed)
**Decision:** The coverage vocabulary shipped at 7 values is reconciled to a Brock-approved **14**: `state_regulated_commercial`, `erisa_self_funded`, `medicare_traditional`, `medicare_advantage`, `medicaid_ffs`, `medicaid_mco`, `dual_eligible`, `self_pay`, `tricare`, `va_champva`, `fehb_pshb`, `nonfederal_governmental`, `stldi`, `excepted_coverage` — plus `all` as a schema-only wildcard. One canonical definition (`runtime/app/plan_types.py`) is the single source of truth; the `laws_regulations.json` schema enum and `packages/shared` mirror it, enforced by a drift test. `dual_qmb` → `dual_eligible` everywhere (migration 0028). Coverage carries a typed **attributes** JSON field (`qmb_status`, `ihs_prc_eligible`, `grandfathered`, `market_segment`, `church_plan`, `medigap`, `dsnp`, `governmental_fully_insured`), validated for key/regime compatibility — not buckets; a null value means "unknown" and the QMB never-bill check keys on `qmb_status IS TRUE` only. Deterministic detection gains FEHB/PSHB, STLDI (the mandated "THIS IS NOT QUALIFYING HEALTH COVERAGE" notice), excepted-benefits, grandfathered (→ attribute), and non-federal-governmental signals; PACE routes to a graceful-handoff seam (no enum value); no MMP value (demo dead 2025-12-31). Hard rules: a **state** law entry may **never** bind `fehb_pshb` (FEHBA preempts state insurance law, 5 U.S.C. 8902(m)(1)) — enforced by `check_state_seed.py`; and `stldi`/`excepted_coverage` audits **suppress** No Surprises Act + ACA-appeal claims (those protections don't apply). The STLDI max-duration threshold is a state-dependent, re-verifiable year-versioned constant (`plan_constants.py`, `amount=None` + `as_of`/source), never hardcoded — the federal 4-month rule is under non-enforcement since 2025-08-07 with litigation stayed.
**Reasoning:** the 7-value set collapsed distinctions that change the legal answer (Medicaid FFS vs MCO, TRICARE vs VA/CHAMPVA, FEHB's federal preemption, STLDI/excepted sitting outside the ACA/NSA). Brock split them so the audit never asserts a protection the population doesn't have. Excepted coverage inherits the `self_pay` rule-set (GFE/PPDR) plus a pursue-reimbursement step, expressed as an explicit regime→ruleset mapping rather than a copy.
**Reversibility:** the enum is locked by Brock's memo; new populations append to `plan_types.py` (+ migration + the mirrors, which the drift test guards). Attribute keys extend the same way. TODO(brock-content): the STLDI per-state duration numbers + any population-specific rules corpora (the audit runs the generic commercial path with an explicit assumption until each lands).

## DL-91 — chat-first audit flow (Phase A) behind ENABLE_CHAT_FIRST_AUDIT

**Date:** 2026-07-10 · **Decided by:** Brock (design doc 2026-07-10, binding decisions) · **Phase:** A (dev-only, flag-gated)
**Terminology:** in code a `case` == a product **sub-case**; the user-level "Tyndale Record" view is **Phase C** (D5, out of scope here).
**Decision:** The audit is presented as a single chat thread instead of the classic screen sequence. Phase A ships behind `enable_chat_first_audit` (default false, dev-only); the classic encounter/results flow is fully retained and unchanged when the flag is off (D7 — flag-off *is* the transition; nothing is retired yet). Binding decisions, encoded:
- **D0** — the three-number reveal and the first-case-unlock render as full-width, visually distinct **moment cards**, never ordinary bubbles.
- **D1** — ALL system-authored thread copy comes from a versioned doctrine file, `intelligence-layer/prompts/orchestration_script.md` (`## key` registry + `version` frontmatter, loaded by `context_loader.load_orchestration_script`/`orchestration_step` and rendered verbatim). Engineering seeds `[PLACEHOLDER-eng]`-prefixed values for dev; `assert_production_safety` FAILS a **staging or production** boot while any active value still carries that prefix (new `is_staging_or_prod`).
- **D2** — minimal chattiness: ONE live status card updated in place, four labeled flow-stage bars (Extraction / Line-items / Encounter / Audit) filling on REAL stage completion, no fabricated percentages. *Engineering interpretation (flagged for Brock):* the four bars are the four FLOW stages, each derivable from case status — the four audit *sub-agents* aren't persisted, so rendering them would require fabricating progress; using flow stages keeps the thread a pure projection of case state (the harness invariant).
- **D3** — verification cards grouped ≤3 per message.
- **D4** — Phase A shipped option (a): structured taps only, with a script-voiced "tap to confirm" nudge. **Phase B (2026-07-10) ships option (b):** a free-text reply routes to `POST /audit/{id}/verify-text`, the `verification_mapper` (deterministic layer — ordinals / code / amount / universals / negations — then Haiku, failure-safe) maps it to the card(s) + intended answer, and the thread renders a PRE-SELECTABLE suggestion. The invariant is absolute: **the tap is the only thing that changes state** — free text never writes to `/confirmations`; a mapped-but-unconfirmed card persists nothing; the mapper only pre-selects on a high-confidence, complete mapping (any low-confidence or partial → the tap nudge, never a silent subset). Chat ingress (crisis, then injection) runs before the mapper. Mixed messages ("…wrong — also what's a deductible?") are map-only in Phase A (the question is not auto-answered). New script keys `verification_map_confirm`/`_fallback`/`_partial_fallback`; new thread kind `verification_suggestion` (migration 0031).
- **D6** — a cap collision mid-audit renders the script-voiced `cap_collision` line, never a raw rate-limit error.
**Architecture:** a server-side **event bridge** (`app/agents/thread_bridge.py`) renders orchestrator case-state transitions into typed chat-thread entries — a new `Message.kind` discriminator (`status_card_update` / `system_message` / `moment_card` / `verification_request`, migration 0030) + `payload` JSONB, authored as `role='system'` rows. The bridge holds **no state machine**: the thread is a pure, idempotent projection of `(status, line_items, findings, …)`, so it is correct even when nobody is connected and reconstructable at any time (the harness asserts this). Hooked fire-and-forget from `_set_status` (+ the EOB-completeness bypass writer); flag-gated + error-swallowing so it can never break the audit path. On upload, a new case bootstraps a thread (acknowledgment + status card) and the client routes there. Liveness in Phase A is **polling** of the conversation detail — no out-of-band SSE push exists yet; a dedicated per-conversation subscription stream is a Phase-B fast-follow.
**Out of Phase A:** D4(b) free-text mapping · the Record/sub-case navigation (D5, Phase C) · billing/pricing rework — the unlock moment card is built to Brock's structure but rendered only behind `enable_billing && enable_first_case_unlock`, **both false** (server never emits it; pricing memo pending: first-case $4.99 with full follow-through, subscription gates case count not mid-case abandonment) · retiring standalone routes (at flip) · deep-link redirects (Phase C). No Stripe changes.
**Reversibility:** the whole surface is one flag; flipping `enable_chat_first_audit` off restores the classic flow with zero migration. TODO(brock-content): replace every `[PLACEHOLDER-eng]` value in `orchestration_script.md` with authored copy before staging (the boot guard enforces it).

## DL-91 (Phase C) — the Tyndale Record + sub-case views (D5) behind ENABLE_RECORD_VIEW

**Date:** 2026-07-14 · **Decided by:** Brock (design decision D5) · **Phase:** C (dev-only, flag-gated)
**Decision:** The user-level **Tyndale Record** (product term for the view that groups a person's sub-cases) ships behind `enable_record_view` (default false, **independent** of `enable_chat_first_audit`). Flag-off is a no-op: the dashboard's ad-hoc Open Cases card renders exactly as before. Built:
- **§1 Record** — `GET /v1/record` (rolling, parameterizable window; default 12mo, `has_older` flags older cases) returns per-sub-case rows (status, three-number brief, open-item count, next deadline, recovered-so-far) + Record-level aggregates. When the flag is on, the mobile dashboard shows the Record in place of the Open Cases list; coverage meters stay on top. Two designed continuous-journey moments render in the chat-first thread via the bridge: `record_first_upload_frame` (first-ever upload) and `record_post_audit_keep_doing` (once, post-terminal).
- **§2 Sub-case summary** — `GET /v1/case/{id}/summary`: status banner + response-deadline clock (persisted rows only; shadow-mode OK to display), the three-number moment re-hosted from Phase A, recovered-so-far (CONFIRMED) beside identified (ESTIMATE, labeled separately), needs-documents have/need checklist, next check-in, and the **gameplan** — actionable findings ordered biggest-dollar-first, each a per-call script with four beats (when they pick up / the problem / the ask / get it in writing) + an "if they push back" contingency. Engineering renders the structure; connective copy is D1 orchestration-script keys (`call_script_*`, `call_mode_*`); the problem/ask are the finding's own Tier-B claim / Tier-C action. Mobile `/case/{id}` screen + a presentation-only, full-screen **call mode** step-through that writes nothing (the dashboard's outcome follow-up captures results later).
- **§3 Navigation** — a deep link to legacy `/audit/{id}` redirects to the canonical Record surface when the flag is on (results-bearing → `/case/{id}`, in-flight → `/audit/{id}/thread`); non-blocking, so the flag-off classic screen is unaffected. `/record` + `/case/*` join `/audit/*` as **case-work routes** exempt from BOTH the intake and profile gates (`isCaseWorkRoute`) — an existing user deep-linking there is never bounced into a wizard/onboarding loop (the 2026-07-06 re-gating class).
- **§4 Data honesty (tests)** — recovered/avoided comes ONLY from CONFIRMED `outcome_report` events (resolved yes|partial, latest-per-case); the finding gap is 'identified', shown separately and never summed into recovered; a case with no computable three-number carries None, never `{0,0,0}`; deadline clocks come only from persisted rows with a provenance source. `identified_estimate_from_findings` sums every finding's positive gap (payer- and provider-side alike) so the Record aggregate, the summary, and the gameplan's per-step figures agree — the Independent Audit Doctrine pursues both sides equally.
**Harness:** `run_scenarios.py --record` asserts each sub-case appears in `/v1/record` with the right status and honest $0-recovered (no outcome confirmed → an estimate leaking into recovered would show here), plus a suite-level ≥2-rows/sane-aggregates check. Requires `ENABLE_RECORD_VIEW` on the target (the flag-off 404 is covered by unit tests).
**Out of Phase C:** billing/entitlement gating (the mocked $4.99 case page ships UNgated in dev) · Complete-tier features · SMS · route retirement (Phase D) · Record export/sharing.
**Reversibility:** one flag (`enable_record_view`); off restores the classic dashboard with zero migration. TODO(brock-content): author the `call_script_*` / `call_mode_*` / `record_*` `[PLACEHOLDER-eng]` values before staging (the boot guard enforces it).

## DL-92 — internal analytics P0: four binding design rules (Brock's dashboard spec 2026-07-11)

**Date:** 2026-07-11 · **Decided by:** Brock (dashboard spec) · **Scope:** P0 (event stream + aggregation + admin dashboard + substantiation scaffolding). P1/P2 dashboards deferred; billing-dependent events registered, not emitted.
**Four rules, encoded as constraints (not conventions):**
- **R1 — every rate names its denominator.** `analytics_daily` stores numerator, denominator, and a pinned `definition` per metric; a CHECK constraint refuses a blank definition, and `MetricDef` refuses one at construction. The admin API returns n/d + definition with every ratio, and the dashboard renders the raw n/d beside every %. Metric definitions live in ONE module (`app/analytics/definitions.py`); the win-rate definition is verbatim **resolved ÷ outcomes REPORTED, never ÷ all cases**, with `outcome_report_rate` defined beside it and hard-paired in the UI (win rate can't render without its basis).
- **R2 — PHI-free by construction.** ONE `analytics_events` table; properties validate against a per-event schema of **enums / numbers / booleans only** (`app/analytics/events.py`). There is no free-text property type, so an un-enumerated string is unrepresentable — "a string type must be an enum" is structural. `validate_event` rejects unregistered names and non-conforming values; UTM capture is sanitized to closed enums (raw campaign is dropped, not stored).
- **R3 — first-party only.** Plausible removed from the authenticated member app (no self-hosted instance; kept on the unauthenticated marketing site). Funnel truth is emitted **server-side** wherever the fact is server-known (`app/analytics/emit.py`), never trusted from the client; `POST /v1/events` accepts only genuinely client-side events and drops server-only/unknown/not-yet-live names with a counter.
- **R4 — counts before ratios.** A panel-wide rendering rule enforced in the admin dashboard.
**Architecture:** append-only event stream (migration 0032) → nightly `analytics_rollup` cron into `analytics_daily` (0033) → `GET /v1/admin/analytics` (admin-gated) → the admin `/analytics` dashboard. A source-table backfill (`scripts/backfill_analytics.py`) seeds history, flagged `backfilled`. Outcome capture is **idempotent** (a `dedupe_key` unique index): a double-tapped outcome button can never double-report (the P0 reliability guarantee). Substantiation export (`scripts/generate_substantiation.py`, §7) gates each public claim on a minimum sample size and writes dated JSON+Markdown — gates unmet → **NOT PUBLISHABLE** with the shortfall; the win-rate export hard-embeds its qualifier. No public surface is published here.
**Emit coverage:** live — the funnel backbone, outcome idempotency, close-the-loop, engagement/accuracy events, the §6 compliance counters (crisis at both chat + verify-text ingress, injection→refusal, consent opt-in/withdraw, self-serve deletion), the mapper suggested/fallback events, and `stage_completed{audit}` latency. Still deferred (each needs more than a clean server chokepoint, not a registry gap): `reveal_viewed` (its "on fetch" trigger would write on every poll — needs a non-polling signal), `reaudit_delta` (needs the prior three-number captured before the re-run), `mapper_confirmed` (needs a client signal distinguishing a confirmed-suggestion tap), `signup_attribution` (needs UTM plumbing marketing→app→API), and the extraction/translate/encounter flow-stage timings. Billing events stay registered/not-live until the pricing rework.
