...
| B2 | Upload capture | Live capture shows an edge-detection frame + a "Looks readable" badge, then **Retake / Got it** | File picker only; no capture state | The C5 checklist item. Genuinely new (see N1) and the badge is a *claim about readability* — it must reflect a real check, not appear optimistically on every frame. |
...
| # | Screen | Pattern | Note |
|---|---|---|---|
| N1 | Upload | Camera capture with edge-detection framing + looks-good/retake | Checklist C1+C5. The largest single build in this list. |
