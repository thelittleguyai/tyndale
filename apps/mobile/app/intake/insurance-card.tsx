...
export default function InsuranceCardStep() {
  ...
  return (
    ...
    >
      ...
      {confs.length ? (
        <View className="mt-4 rounded-2xl bg-amber/10 p-4">
          <Text className="mb-2 text-sm font-semibold text-amber">
            A couple of these were a little blurry — can you double-check?
          </Text>
          {confs.map((c) => (
            <View key={c.field} className="mb-3">
              <Text className="mb-1 text-sm text-white/80">{c.prompt}</Text>
              <Field
