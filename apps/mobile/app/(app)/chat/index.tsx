    ...
    <View className="flex-1 bg-navy-deep">
      <View className="flex-row items-center justify-between border-b border-white/10 bg-navy-soft px-4 py-3">
        <Link href="/" asChild>
          <Pressable className="flex-row items-center gap-1">
            ...
            <Text className="text-sm text-white/70">Home</Text>
          </Pressable>
        </Link>
        <Text className="text-base font-bold text-white">Ask Tyndale</Text>
        <Pressable
          onPress={start}
          className="flex-row items-center gap-1 rounded-full bg-sage px-3 py-1.5"
        >
      ...
      <ScrollView className="flex-1 px-4" contentContainerStyle={{ paddingVertical: 16 }}>
        ...
        {!loading && items.length === 0 ? (
          <View className="mt-6 rounded-2xl border border-white/10 bg-navy-soft p-5">
            ...
            <Pressable onPress={start} className="mt-4 self-start rounded-xl bg-sage px-4 py-2.5">
        ...
        {items.map((c) => (
          <Link key={c.conversation_id} href={`/chat/${c.conversation_id}`} asChild>
            <Pressable className="mb-2 flex-row items-center gap-3 rounded-xl border border-white/10 bg-navy-soft p-4">
