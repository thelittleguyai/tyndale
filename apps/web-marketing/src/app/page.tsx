import { Clock, FileText, Lock, MessageSquare, Star, Zap, type LucideIcon } from 'lucide-react';
import { GoogleCta } from '@/components/google-cta';
import { GoogleIcon } from '@/components/google-icon';
import { Logo, Wordmark } from '@/components/logo';

type Feature = { icon: LucideIcon; title: string; body: string };

// Icons matched to docs/design/marketing_landing.png. Note: the screenshot uses
// a clock for BOTH "Easy Sign-In" and "Always Up to Date" — matched as drawn.
const FEATURES: Feature[] = [
  {
    icon: Clock,
    title: 'Easy Sign-In',
    body: 'Log in with your Google account in one click. No extra passwords to create or remember.',
  },
  {
    icon: Zap,
    title: 'Simple to Use',
    body: 'A clean, intuitive design that makes managing your health information quick and effortless.',
  },
  {
    icon: FileText,
    title: 'Insurance Card Scanner',
    body: 'Snap a photo of your insurance card and we automatically read and save your coverage details.',
  },
  {
    icon: Lock,
    title: 'Your Data Stays Safe',
    body: 'Your personal and health information is protected with industry-standard security at every step.',
  },
  {
    icon: MessageSquare,
    title: 'AI Health Assistant',
    body: 'Ask questions, review bills, or get guidance — our built-in assistant is here to help anytime.',
  },
  {
    icon: Clock,
    title: 'Always Up to Date',
    body: 'See your coverage details, cost estimates, and account activity in real time.',
  },
];

const BADGES = [
  { title: 'Fast', caption: 'LOADS IN UNDER A SECOND' },
  { title: 'Secure', caption: 'ENCRYPTED & PROTECTED' },
  { title: 'Reliable', caption: 'ALWAYS AVAILABLE' },
];

export default function HomePage() {
  return (
    <>
      {/* ── Top: navy→teal gradient spanning header + hero (sampled from the screenshot) ── */}
      <div className="bg-gradient-to-b from-[#212D3F] via-[#2C4D52] to-[#3D636A]">
        <header>
          <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-5 text-white">
            <div className="flex items-center gap-2.5">
              <Logo size={34} />
              <Wordmark className="text-white" />
            </div>
            <a
              href="/signin"
              className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-medium text-ink transition hover:bg-white/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
            >
              <GoogleIcon size={16} />
              Sign In
            </a>
          </div>
        </header>

        <section>
          <div className="mx-auto max-w-6xl px-6 pb-24 pt-12 sm:pt-16">
            <div className="max-w-2xl">
              <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-1.5 text-xs font-semibold uppercase tracking-[0.12em] text-white/90">
                <Star size={12} strokeWidth={0} fill="currentColor" className="text-[#74D89E]" />
                AI-Powered Health Platform
              </span>
              <h1 className="mt-6 text-5xl font-bold leading-[1.05] tracking-tight text-white sm:text-6xl">
                Tyndale.
                <br />
                <span className="font-normal text-white/80">Welcome to the App.</span>
              </h1>
              <p className="mt-6 max-w-md text-lg leading-relaxed text-white/65">
                Sign in with your Google account to get started. New users will be asked to
                complete a quick profile setup.
              </p>
              <div className="mt-9 flex flex-wrap items-center gap-5">
                <GoogleCta />
                <a
                  href="#features"
                  className="text-sm font-medium text-white/80 transition hover:text-white"
                >
                  Learn more →
                </a>
              </div>
            </div>
          </div>
        </section>
      </div>

      <main>
        {/* ── Features (cream) ── */}
        <section id="features" className="bg-cream">
          <div className="mx-auto max-w-6xl px-6 py-20 sm:py-24">
            <div className="text-center">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-teal">Features</p>
              <h2 className="mt-4 text-3xl font-bold tracking-tight text-ink sm:text-4xl">
                Simple. Fast. Secure.
              </h2>
              <p className="mx-auto mt-4 max-w-2xl text-lg text-ink/60">
                Everything you need to manage your health information in one place, designed to
                be easy for everyone.
              </p>
            </div>

            <ul className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {FEATURES.map(({ icon: Icon, title, body }) => (
                <li
                  key={title}
                  className="rounded-2xl bg-surface p-7 shadow-card ring-1 ring-black/[0.04]"
                >
                  <span className="inline-flex h-11 w-11 items-center justify-center rounded-xl bg-teal-tint text-teal">
                    <Icon size={21} strokeWidth={1.75} aria-hidden="true" />
                  </span>
                  <h3 className="mt-5 text-base font-semibold text-ink">{title}</h3>
                  <p className="mt-2 text-sm leading-relaxed text-ink/60">{body}</p>
                </li>
              ))}
            </ul>
          </div>
        </section>

        {/* ── "Built for you" (navy) ── */}
        <section className="bg-[#212D3F]">
          <div className="mx-auto max-w-5xl px-6 py-20 text-center sm:py-24">
            <h2 className="text-3xl font-bold tracking-tight text-white sm:text-4xl">
              Built for you
            </h2>
            <p className="mt-4 text-lg text-white/55">
              Designed to work the way you expect — every time.
            </p>
            <div className="mx-auto mt-12 grid max-w-3xl grid-cols-1 divide-y divide-white/10 overflow-hidden rounded-2xl bg-white/[0.04] ring-1 ring-white/10 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
              {BADGES.map(({ title, caption }) => (
                <div key={title} className="flex flex-col items-center px-6 py-8">
                  <span className="text-2xl font-bold text-white">{title}</span>
                  <span className="mt-2 text-xs font-semibold uppercase tracking-[0.12em] text-white/45">
                    {caption}
                  </span>
                </div>
              ))}
            </div>
          </div>
        </section>
      </main>

      {/* ── Footer ── */}
      <footer className="bg-[#1A2532]">
        <div className="mx-auto max-w-6xl px-6 py-12">
          <div className="flex items-center gap-2.5 text-white">
            <Logo size={28} />
            <Wordmark className="text-base text-white" />
          </div>
          <p className="mt-5 max-w-2xl text-sm text-white/55">
            Tyndale provides medical billing and coverage advocacy, not medical, legal, or
            financial advice.
          </p>
          <nav className="mt-6 flex gap-6 text-sm text-white/70" aria-label="Footer">
            <a href="/privacy" className="transition hover:text-white">
              Privacy
            </a>
            <a href="/terms" className="transition hover:text-white">
              Terms
            </a>
            <a href="mailto:support@tyndaleapp.net" className="transition hover:text-white">
              Contact
            </a>
          </nav>
          <p className="mt-8 text-xs text-white/40">© 2026 The Little Guy LLC. d/b/a Tyndale.</p>
        </div>
      </footer>
    </>
  );
}
